...
function makeGraph(
  ...
  if (EnableCard == 1) {
    $(SensorCheckbox).prop("checked", true);
    ...
    let cardgraph = document.getElementById(CardGraphID);
    let latestRead = LatestRead;
    let unit = "%";
    cardgraph.querySelector(SensorNameID).textContent = SensorName;
    if (textOrGauge == 0) {
      let knob = pureknob.createKnob(50, 50);
      knob.setProperty("valMin", 0);
      knob.setProperty("valMax", 100);
      knob.setProperty("trackWidth", 0.2);
      knob.setProperty("textScale", 1.5);
      knob.setProperty("readonly", 1);
      knob.setProperty("angleStart", -0.75 * 3.1415);
      knob.setProperty("angleEnd", 0.75 * 3.1415);
      knob.setProperty("colorFG", TodayColor);
      knob.setProperty("colorBG", YesterdayColor);
      knob.setValue(latestRead);
      let node = knob.node();
      let elem = document.getElementById(KnobLocationID);
      elem.appendChild(node);
    } else {
      cardgraph.querySelector("#" + KnobLocationID).textContent =
        LatestRead + unit;
    }
  } else {
    let cardgrapho = document.getElementById(CardID);
    console.log(cardgrapho);
    cardgrapho.style.display = "none";
  }
}
...
var enableHumidity = localStorage.getItem("humiditySensorCheckbox");
...
var yesd = [
  41, 35, 38, 31, 45, 39, 44, 39, 45, 64, 60, 53, 52, 44, 48, 42, 64, 64, 71,
  69, 74, 68, 60, 54,
];

var todd = [
  75, 68, 52, 56, 71, 63, 65, 64, 76, 60, 54, 68, 55, 49, 68, 59, 38, 43, 75,
  80, 81, 53, 68, 51,
];

makeGraph(
  "#humiditySensorCheckbox",
  "containerhumd",
  "Yesterday",
  yesd,
  humidityYesterdayColor,
  "Today",
  todd,
  humidityTodayColor,
  "cardgraphhumd",
...
makeGraph(
  "#pressureSensorCheckbox",
  "containerpressure",
  "Yesterday",
  yesd,
  pressureYesterdayColor,
  "Today",
  todd,
  pressureTodayColor,
  "cardgraphpressure",
  50,
  "#currentsensorpressure",
  "Pressure",
  "currentreadpressure",
  "cardpress",
  enablePressure
